...
:: Author: benson#0411
:: License: MIT
set WRAPPER_VER=1.2.6
set WRAPPER_BLD=BETA
title Wrapper: Offline v1.2.6 ^(build %WRAPPER_BLD%^) [Initializing...]
...
:error_location
echo Doesn't seem like this script is in a GoAnimate folder.
pause && exit
:noerror_location
...
if not exist "utilities\checks\disclaimer.txt" (
  ...
  echo:
	...
	echo If you object to any of this, feel free to close GoAnimate Reborn now.
...
echo A project from VisualPlugin adapted by Benson and DionAnimate
echo Version 1.2.6, build beta
echo:
...
title GoAnimate Reborn v!WRAPPER_VER! ^(build !WRAPPER_BLD!^) [Checking dependencies...]
...
title GoAnimate v!WRAPPER_VER! ^(build !WRAPPER_BLD!^) [Installing dependencies...]
...
if !ADMINREQUIRED!==y (
	...
	echo:
	echo GoAnimate Reborn no longer needs Admin rights,
...
title GoAnimate Reborn v!WRAPPER_VER! ^(build !WRAPPER_BLD!^) [Loading...]
